...
def data_validation(data_dict: dict):
    ...
    #data_dict['PERFECT'] = data_dict['perfect'].upper() == TRUE
    ...
    if (',' in data_dict['ENTRY'] and ',' in data_dict['EXIT']) is False:
        raise ErrorInConfigFile(
                        ...
                        "X,Y")
    #maybe we should handle space between commas: (1_,_1)   -> 2 spaces
    data_dict['ENTRY'] = (data_dict['ENTRY'].split(','))
    data_dict['EXIT'] = (data_dict['EXIT'].split(','))

    if ((data_dict['ENTRY'][0].isdigit() and data_dict['ENTRY'][1].isdigit())   #data_dict['ENTRY'][0].strip().isdigit()
            is False):
        raise ErrorInConfigFile(f"Invalid ENTRY coordinates: {data_dict['ENTRY']}."
                                " Coordinates must be numbers.")
    if ((data_dict['EXIT'][0].isdigit() and data_dict['EXIT'][1].isdigit())     #data_dict['EXIT'][0].strip().isdigit()
            is False):
    ...
    if (data_dict['ENTRY'][0] == data_dict['EXIT'][0] and
            data_dict['ENTRY'][1] == data_dict['EXIT'][1]):         #it should be or instead of and! maybe?!
